...
PTCB* acquire_PTCB()
{
	PTCB* ptcb = (PTCB*)xmalloc(sizeof(PTCB)); // space for ptcb
  return ptcb;
}
/* 
	Initialize ptcb
*/
void initialize_ptcb(PTCB* ptcb, Task task, int argl, void* args)
{
  ptcb->task = task;
  ptcb->argl = argl;
  ptcb->exited = 0;
...
/* Variable to count the times yield is called */
...
static TCB* sched_queue_select(TCB* current)
{
	...
	TCB* next_thread = (current->state == READY) ? current : &CURCORE.idle_thread;
	next_thread->its = QUANTUM;
	return next_thread;
...
void yield(enum SCHED_CAUSE cause)
{
	...
	/* Checking if yield is called enough times */
...
{
	if (current_tcb->priority > 0)
	{
		if (cause == SCHED_QUANTUM)
			current_tcb->priority--;
		else if ((current_tcb->curr_cause == cause) && (current_tcb->last_cause == SCHED_MUTEX))
			current_tcb->priority--;
	}
	if (current_tcb->priority != PRIORITY_QUEUES - 1)
	{
		if (cause == SCHED_IO)
			current_tcb->priority++;
	}
