...
class DDPMScheduler(BaseScheduler):
    ...
    def step(self, x_t: torch.Tensor, t: int, eps_theta: torch.Tensor):
        ...
        device = x_t.device
        # Create a tensor of timesteps for batch processing
        t_tensor = torch.full((x_t.size(0),), t, dtype=torch.long, device=device)

        # Retrieve the corresponding coefficients for the current timestep
        ...
        mean = (x_t - (1 - alpha_t) / torch.sqrt(1 - alpha_cumprod_t) * eps_theta) / torch.sqrt(alpha_t)

        # Generate noise only if t > 0, otherwise use zeros
