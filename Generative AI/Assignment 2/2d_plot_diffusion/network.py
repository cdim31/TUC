...
class SimpleNet(nn.Module):
    def __init__(
        ...
        # Create a sequence of TimeLinear layers for each hidden dimension
        layers = []
        prev_dim = dim_in
        for hidden_dim in dim_hids:
            layers.append(TimeLinear(prev_dim, hidden_dim, num_timesteps))
            prev_dim = hidden_dim
        self.time_linears = nn.ModuleList(layers)
        # Add a final linear layer to map to the output dimension
        self.final_linear = nn.Linear(prev_dim, dim_out)
        ######################
    ...
    def forward(self, x: torch.Tensor, t: torch.Tensor):
        ...
        for layer in self.time_linears:
            x = layer(x, t)
            x = F.silu(x)
